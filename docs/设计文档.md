...
bpt9: key: username. value: 所有已购车票信息。如 query_order 所述。
...
等候队列 bpt10: 以 pair(trainID，发车时间) 为 key，按顺序记录等候队列。

refund_ticket: 找对应车次，把票加回来，然后一个个跑等候队列，如果能走就走。走了把等候队列给删掉，去 query_order(bpt9) 里面改状态。